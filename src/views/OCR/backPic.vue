      ...
      <div class="card-box">
        <van-uploader :after-read="onReadBackFile" accept="image/*" class="upload-box">
          <div class="ID-icon" v-if="!backPicUrl">
            <div class="mask"></div>
            <div class="icon-box">
              <van-loading color="white" v-if="backLoading" />
              <template v-else>
                <i class="Icon-Loan iconID-cardnegative icon-idcard"></i>
                <p class="fs14">请上传反面身份证</p>
              </template>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    async handerUpload () {
      ...
      formdata.set('productId', this.$route.query.productId);
      formdata.set('picType', 'B');
